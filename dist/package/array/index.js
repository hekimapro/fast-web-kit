 ...
 */
export const hasSameDataType = (arr, type) => hasElements(arr) && type ? arr.every(item => type === "array" ? isValid(item) : typeof item === type) : false;
/**
 * Check if a given array has falsy values
 * @param arr The array to check
 * @returns true if the array has falsy values, false otherwise
 */
export const hasFalsyValues = (arr) => hasElements(arr) ? arr.some(item => !item) : false;
/**
 ...
 */
export const elementExist = (arr, elem) => hasElements(arr) ? arr.includes(elem) : false;
/**
 ...
 */
export const getElementIndex = (arr, elem) => elementExist(arr, elem) ? arr.indexOf(elem) : -1;
/**
 ...
 */
export const removeElement = (arr, elem) => {
    try {
 ...
 */
export const removeFalsyElements = (arr) => hasElements(arr) ? arr.filter(Boolean) : [];
/**
 ...
 */
export const elementsWithSameType = (arr, dataType) => hasElements(arr) && dataType ? arr.filter((element) => dataType === "array" ? isValid(element) : dataType === "null" ? element === null : ((typeof element === dataType) && (element !== null))) : [];
/**
 ...
 */
export const countOccurrences = (arr, elem) => {
    try {
        if (elementExist(arr, elem)) {
 ...
 */
export const computeMathOperation = (nums, operator) => {
    try {
...
*/
export const getMiddleElement = (arr) => {
    try {
        if (hasElements(arr)) {
 ...
 */
export const getElementByIndex = (arr, index) => {
    try {
 ...
 */
export const elementsExist = (arr1, arr2) => {
    try {
        // Validate function parameters
